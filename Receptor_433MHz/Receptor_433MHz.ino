...
void setup() {
  ...
  vw_set_rx_pin(3);         //Pin 2 is connected to "Digital Output" of receiver
...
void loop() {
  ...
  if (vw_get_message(buf, &buflen)) { // Non-blocking
    ...
    for (int i = 0; i < buflen; i++) {
      ...
      //Serial.print(" ");
      if (buf[i] == '*') {
        id = ((buf[i + 1] - 48) * 10 + buf[i + 2] - 48);
        minutos = buf[i + 4] - 48;
        segundos = ((buf[i + 6] - 48) * 10 + buf[i + 7] - 48);
        decimas = buf[i + 9] - 48;
        numVueltas = buf[i + 11] - 48;
      }
    ...
    Serial.println();
    Serial.print("Datos decodificados ID:");
    Serial.print(id);
    Serial.print(" Tiempo:");
